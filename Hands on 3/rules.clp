...
(defrule razer-pc-video-game-discount-videogames-retail
    (customer (name ?name) (email ?email) (phone ?phone) (address ?address))
    (order (order-id ?order-id) (customer-email ?email) (order-date ?order-date) (delivery-date ?delivery-date))
    ;(computer (id 18) (brand "Razer") (model "Blade Stealth 13") (price 1799) (color "Black") (storage "512GB SSD") (ram "16GB") (stock 3))
    (order-item (order-id ?order-id) (product-id 18) (product-type "computer") (quantity ?quantity&:(<= ?quantity 10)) (price-per-unit ?price-per-unit))
    (computer (id 18) (brand "Razer") (model "Blade Stealth 13") (price ?price) (color "Black") (storage "512GB SSD") (ram "16GB") (stock ?stock))
    (order-item (order-id ?order-id) (product-id ?product-id-vg))
    => 
    (if (>= ?stock ?quantity) then
        (printout t "------------------------------" crlf)
        (printout t "Retail" crlf)
        ...
        (printout t "Phone: " ?phone crlf)
        (printout t "Address: " ?address crlf)
        (printout t "Stock Razer Blade Stealth 13 Black 512GB SSD 16GB: " (- ?stock ?quantity) crlf)
        ...
        (printout t "Recommended Product: The Witcher 3: Wild Hunt" crlf)
        (assert (recommendation (customer-email ?email) (product-id 69)))
        (printout t "Recommended Product: Hades" crlf)
        (assert (recommendation (customer-email ?email) (product-id 70)))
        (printout t "Recommended Product: Stardew Valley" crlf)
        (assert (recommendation (customer-email ?email) (product-id 71)))
        (printout t "Discount 10% on video games" crlf)
        (assert (discount-type-product (product-type "videogame") (percentage 10) (customer-email ?email)))
        (printout t "------------------------------" crlf)
    else
        (printout t "------------------------------" crlf)
        (printout t "Stock not available for Razer Blade Stealth 13 Black 512GB SSD 16GB" crlf)
        (printout t "------------------------------" crlf)))

(defrule apply-discount-videogames
    (customer (name ?name) (email ?email) (phone ?phone) (address ?address))
    (order (order-id ?order-id) (customer-email ?email) (order-date ?order-date) (delivery-date ?delivery-date))
    (order-item (order-id ?order-id) (product-id ?product-id) (product-type "videogame") (quantity ?quantity) (price-per-unit ?price-per-unit))
    (discount-type-product (product-type "videogame") (percentage ?percentage) (customer-email ?email))
    =>  
    (printout t "------------------------------" crlf)
    (printout t "Customer: " ?name crlf)
    (printout t "Email: " ?email crlf)
